...
function renderTopBar() {
  ...
  document.getElementById('score-display').textContent = state.totalScore.toLocaleString();
  document.getElementById('score-max').textContent = `/ ${state.maxScore.toLocaleString()}`;
...
function showLevelComplete() {
  ...
  document.getElementById('lo-score').textContent = `${state.totalScore.toLocaleString()} punkti`;
...
function showBonusChoice() {
  hideLevelOverlay();
  document.getElementById('bo-score').textContent = `${state.totalScore.toLocaleString()} punkti`;
...
function showFinalResults() {
  ...
  if (state.gameMode === 'party' && state.party) {
    ...
    document.getElementById('fo-score').textContent = `${p.names[0]} ${p.scores[0].toLocaleString()} · ${p.names[1]} ${p.scores[1].toLocaleString()}`;
  ...
  document.getElementById('fo-score').textContent =
    `${state.totalScore.toLocaleString()} / ${state.maxScore.toLocaleString()} punkti`;
  document.getElementById('fo-title').textContent = final.title;
  document.getElementById('fo-msg').textContent = `${final.msg} Ātruma bonuss: +${state.totalTimeBonus} punkti. Kopējais laiks: ${formatDuration(state.timerSeconds)}.`;
...
function updateStartPaths(mode) {
  ...
  actions.classList.toggle('party-only', isParty);
  participationLabel.hidden = isParty;
  researchBtn.hidden = isParty;
  regularBtn.querySelector('strong').textContent = isParty ? 'Ballītes versija' : 'Spēlētājs';
  regularBtn.querySelector('small').textContent = isParty ? 'Sākt sacensību' : 'Spēlē';
}
...
function parseMode(value) {
  return value === 'daily' || value === 'party' ? value : Number(value);
}

function wireModeCards() {
  document.querySelectorAll('.mode-card').forEach(card => {
    card.addEventListener('click', () => {
      selectMode(parseMode(card.dataset.mode));
    });
  });
  setParticipationMode('player');
  ...
  document.getElementById('research-start-btn').addEventListener('click', () => {
    if (state.selectedMode === null || state.selectedMode === undefined || state.selectedMode === 'party') return;
...
function checkContinuePrompt() {
  ...
  if (desc) {
    desc.textContent =
      `${MODE_NAMES[data.gameMode] || ''} · ${data.currentLevel || 1}. līmenis · ${(data.totalScore || 0).toLocaleString()} punkti`;
  }
