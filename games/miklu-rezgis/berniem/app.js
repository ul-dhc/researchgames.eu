...
function solveRiddle() {
  ...
  renderAnswerBoxes();
  showMessage(`Tu atminēji! ${'★'.repeat(stars)}`, 'success');
...
function renderRiddle() {
  ...
  el('stars-total').textContent = `${state.totalStars} ${state.totalStars === 1 ? 'zvaigzne' : 'zvaigznes'}`;
...
function finishGame() {
  ...
  const maxStars = GAME_LENGTH * 3;
  const title = state.totalStars >= 13 ? 'Izcili!' : state.totalStars >= 9 ? 'Lieliski!' : 'Labs darbs!';
  el('result-title').textContent = title;
  el('result-stars').textContent = `${state.totalStars} ★`;
  el('result-copy').textContent = `Tu atminēji visas 5 mīklas un ieguvi ${state.totalStars} no ${maxStars} zvaigznēm.`;
