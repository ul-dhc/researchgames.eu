(() => {
  ...
  function buildLivingField(){
    ...
    const cols=16, rows=9, colors=['#725cff','#b457df','#4f87ff'];
    for(let i=0;i<cols*rows;i++){
      const card=cards[i%cards.length], cardBox=card.getBoundingClientRect();
      const sx=cardBox.left-gridBox.left+cardBox.width/2, sy=cardBox.top-gridBox.top+cardBox.height/2;
      ...
      dot.style.setProperty('--delay',`${260+(i%16)*22+Math.floor(i/16)*42}ms`);
