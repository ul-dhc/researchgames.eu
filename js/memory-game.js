(() => {
  ...
  const mobileLabelLines = isLatvian
    ? { classification:['Klasifi','cēšana'], reconstruction:['Rekonstru','ēšana'] }
    : { classification:['Classifi','cation'], reconstruction:['Recon','struction'] };
  const formatCardLabel = item => {
    const lines = mobileLabelLines[item.key];
    return lines
      ? `<span class="card-label card-label-split"><span>${lines[0]}</span><span>${lines[1]}</span></span>`
